  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 glass-card rounded-3xl p-6 relative overflow-hidden">
        <div>
          <h2 className="text-xl sm:text-2xl font-black font-display tracking-tight flex items-center gap-2" style={{ color: "var(--text)" }}>
            <Layers className="w-5 h-5 text-purple-500" /> Integrations <span className="badge-glow text-[10px] px-2.5 py-0.5 font-bold font-sans">Vite Core Marketplace</span>
          </h2>
          <p className="text-xs font-sans mt-1" style={{ color: "var(--text-muted)" }}>Connect Autofy with your favorite business tools to drive automated messaging and payment settlements.</p>
        </div>

        {/* Live system health pulse badge */}
        <div className="flex items-center gap-3 bg-[var(--bg-elevated)] border border-[var(--border)] px-4 py-2.5 rounded-2xl">
          <div className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
          </div>
          <div className="font-sans">
            <p className="text-[10px] text-[var(--text-muted)] uppercase font-black text-[var(--text-muted)] tracking-wider">Ecosystem Status</p>
            <p className="text-[11px] text-[var(--text)] font-bold flex items-center gap-1 mt-0.5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[var(--bg)]/80 border border-[var(--border)] rounded-2xl p-5 hover:border-blue-500/20 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Connected Apps</span>
            <CheckCircle2 className="w-4 h-4 text-emerald-500" />
          </div>
          <h4 className="text-3xl font-black text-[var(--text)] font-mono mt-3">{stats.connected}</h4>
          <p className="text-[9px] text-[var(--text-subtle)] mt-1">active pipeline channels</p>
        </div>

        {/* Card 2: Available marketplace catalog */}
        <div className="bg-[var(--bg)]/80 border border-[var(--border)] rounded-2xl p-5 hover:border-blue-500/20 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Available Apps</span>
            <Cpu className="w-4 h-4 text-blue-500" />
          </div>
          <h4 className="text-3xl font-black text-[var(--text)] font-mono mt-3">{stats.available}</h4>
          <p className="text-[9px] text-[var(--text-subtle)] mt-1">supported ecosystem nodes</p>
        </div>

        {/* Card 3: Messages processed */}
        <div className="bg-[var(--bg)]/80 border border-[var(--border)] rounded-2xl p-5 hover:border-blue-500/20 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Messages Routed</span>
            <MessageSquare className="w-4 h-4 text-purple-500" />
          </div>
          <h4 className="text-3xl font-black text-[var(--text)] font-mono mt-3">{stats.messages}</h4>
        ...
        <div className="bg-[var(--bg)]/80 border border-[var(--border)] rounded-2xl p-5 hover:border-blue-500/20 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Workflow Nodes</span>
            <Activity className="w-4 h-4 text-amber-500" />
          </div>
          <h4 className="text-3xl font-black text-[var(--text)] font-mono mt-3">{stats.workflows}</h4>
          <p className="text-[9px] text-[var(--text-subtle)] mt-1">active automation steps</p>
        </div>
      ...
      <div className="bg-[#08080a] border border-[var(--border)] rounded-3xl p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--border)] pb-5">
              ...
              <button
                ...
                className={`px-3 py-1.5 text-[10.5px] font-bold rounded-xl transition cursor-pointer font-sans whitespace-nowrap ${
                  activeCategory === tab.id
                    ...
                    : "bg-[var(--bg-elevated)]/40 text-[var(--text-muted)] text-[var(--text-muted)] hover:text-[var(--text)]"
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="relative w-full md:w-64 max-w-sm">
            <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[var(--text-subtle)]">
            ...
            <input
              ...
              className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] pl-9 pr-4 py-2 rounded-xl text-xs text-[var(--text)] focus:outline-none focus:border-blue-500/60 placeholder:text-[var(--text-subtle)]"
            />
          </div>
          ...
          {filteredIntegrations.length > 0 ? (
            filteredIntegrations.map((item) => (
              <div
                ...
                } rounded-3xl p-5 hover:border-[var(--border)] transition-all flex flex-col justify-between gap-5 relative group`}
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      ...
                      className={`text-[9.5px] font-black uppercase tracking-wider font-mono cursor-pointer px-2.5 py-1 rounded bg-[var(--bg-card)] border transition duration-200 ${
                        item.connected 
                          ? "text-emerald-400 border-emerald-500/20 hover:bg-[var(--bg-elevated)]" 
                          : "text-[var(--text-subtle)] border-[var(--border)] hover:text-[var(--text)] hover:bg-[var(--bg-elevated)]"
                      }`}
                    >
                  ...
                  <div className="space-y-1.5 cursor-pointer" onClick={() => setSelectedIntegration(item)}>
                    <h4 className="text-sm font-black text-[var(--text)] font-sans group-hover:text-blue-400 transition-colors flex items-center gap-1.5">
                      {item.name} <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </h4>
                    <p className="text-[11px] text-[var(--text-muted)] text-[var(--text-muted)] font-sans leading-relaxed line-clamp-2">
                      {item.description}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {item.features.slice(0, 2).map((feat, idx) => (
                      <span key={idx} className="bg-[var(--bg-card)] border border-[var(--border)] text-[9px] text-[var(--text-subtle)] font-semibold px-2 py-0.5 rounded-full font-mono">{feat}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[var(--border)]/60 font-sans text-[10.5px]">
                  ...
                  <button
                    ...
                    className="py-2 px-3 bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] text-[var(--text)] font-bold rounded-xl transition text-center cursor-pointer"
                  >
                    Manage
                  </button>
                  ...
                  <button
                    ...
                    className={`py-2 px-3 font-semibold rounded-xl tracking-wide duration-200 transition text-center cursor-pointer ${
                      item.connected
                        ? "bg-red-950/10 hover:bg-red-950/30 text-red-400 hover:text-[var(--text)] border border-red-500/10"
                        : "bg-blue-600 hover:bg-blue-500 text-[var(--text)] font-black"
                    }`}
                  >
            ...
            <div className="col-span-full bg-[var(--bg-elevated)] border border-[var(--border)] rounded-3xl p-12 text-center flex flex-col items-center justify-center space-y-4">
              <span className="p-3 bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text-subtle)] rounded-2xl">
                <AlertCircle className="w-6 h-6 text-[var(--text-subtle)]" />
              </span>
              <div>
                <p className="text-xs font-black text-[var(--text)]">No integrations found</p>
                <p className="text-[10px] text-[var(--text-subtle)] mt-1">Try refining search parameters or choose a different catalog category.</p>
              </div>
            </div>
          )}
        </div>
        ...
        {selectedIntegration && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex justify-end">
            ...
            <motion.div
              ...
              className="w-full max-w-lg bg-[var(--bg-card)] border-l border-[var(--border)] h-full overflow-y-auto p-6 md:p-8 flex flex-col justify-between space-y-6 shadow-2xl relative font-sans text-xs"
            >
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-black text-[var(--text)]">{selectedIntegration.name}</h3>
                      <p className="text-[10px] text-[var(--text-subtle)] uppercase tracking-wider font-extrabold">Ecosystem Namespace Component</p>
                    </div>
                  </div>

                  <button
                    onClick={() => setSelectedIntegration(null)}
                    className="p-2 hover:bg-[var(--bg-elevated)] text-[var(--text-muted)] hover:text-[var(--text)] rounded-lg transition"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-[var(--bg-elevated)] border border-[var(--border)] p-4 rounded-2xl">
                  
                  <div>
                    <span className="text-[9px] text-[var(--text-subtle)] uppercase font-black">Sync Status Log</span>
                    <p className="text-[var(--text)] font-extrabold mt-0.5 flex items-center gap-1.5">
                      <span className={`w-2 h-2 rounded-full ${selectedIntegration.connected ? "bg-green-500 animate-pulse" : "bg-[var(--bg-elevated)]"}`} />
                      {selectedIntegration.connected ? "Active Stream Integration" : "Disconnected Portal"}
                    </p>
                  </div>

                  <div>
                    <span className="text-[9px] text-[var(--text-subtle)] uppercase font-black">Connection Health</span>
                  ...
                  <div className="pt-2 border-t border-[var(--border)]/60">
                    <span className="text-[9px] text-[var(--text-subtle)] uppercase font-black">Latest Sync Capture</span>
                    <p className="text-[var(--text)] font-medium mt-0.5 font-mono">{selectedIntegration.lastSync}</p>
                  </div>

                  <div className="pt-2 border-t border-[var(--border)]/60">
                    <span className="text-[9px] text-[var(--text-subtle)] uppercase font-black">API Status</span>
                    <p className="text-[var(--text)] font-medium mt-0.5 capitalize font-mono text-emerald-450 text-emerald-400">{selectedIntegration.apiStatus}</p>
                  </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-black text-[var(--text)] uppercase tracking-wider text-blue-400">Supported Features</h4>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-black text-[var(--text)] uppercase tracking-wider text-purple-400">Setup Instructions manual</h4>
                  <div className="p-4 bg-[var(--bg-elevated)]/30 border border-[var(--border)] rounded-2xl space-y-3">
                    {selectedIntegration.setupInstructions.map((inst, index) => (
                      <div key={index} className="flex gap-3">
                        <span className="w-5 h-5 shrink-0 rounded-full bg-[var(--bg-card)] border border-[var(--border)] flex items-center justify-center font-mono text-[9.5px] font-black text-blue-400">
                          {index + 1}
                        </span>
                        <p className="text-[var(--text-muted)] leading-normal text-[11px] font-sans pt-0.5">{inst}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-3 pt-4 border-t border-[var(--border)]">
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 text-center rounded-xl font-bold transition text-xs cursor-pointer ${
                      selectedIntegration.connected
                        ...
                        : "bg-blue-600 hover:bg-blue-500 text-[var(--text)] font-black shadow-lg"
                    }`}
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="py-3 px-4 bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] border border-[var(--border)] rounded-xl transition text-center text-[var(--text)] flex items-center justify-center"
      ...
      <div className="bg-[#08080a] border border-[var(--border)] rounded-3xl p-6 space-y-6 font-sans">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-4">
          <div>
            ...
            <p className="text-[10.5px] text-[var(--text-subtle)] font-medium">Design and test multi-step custom WhatsApp message triggers and automated API workflows.</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] text-[var(--text)] hover:text-[var(--text)] border border-[var(--border)] hover:border-[var(--border-strong)] text-xs font-black px-3.5 py-2 rounded-xl transition cursor-pointer flex items-center gap-1"
            >
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-500 text-[var(--text)] text-xs font-black px-4 py-2 rounded-xl shadow-lg transition duration-200 cursor-pointer flex items-center gap-1.5"
            >
              <Play className="w-3.5 h-3.5 text-[var(--text)] fill-white" /> Test flow
            </button>
              ...
              <React.Fragment key={node.id}>
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1 bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] text-[var(--text-muted)] hover:text-[var(--text)] rounded transition"
                  >
                    <Trash2 className="w-3 h-3" />
                  </button>

                  <div className="flex items-center gap-2.5">
                    <span className="p-1.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border)]">
                      <node.icon className="w-3.5 h-3.5" />
                    </span>
                    <div>
                      <span className="text-[8.5px] uppercase font-bold text-[var(--text-subtle)] tracking-wider font-mono">
                        {node.type}
                      </span>
                      <h5 className="text-[11.5px] font-black text-[var(--text)] truncate">{node.title}</h5>
                    </div>
                  </div>

                  <p className="text-[10px] text-[var(--text-muted)] text-[var(--text-muted)] font-medium leading-relaxed mt-2.5 line-clamp-2">
                    {node.desc}
                  </p>

                  {/* Indicator showing current active flow run */}
                  {isHighlighted && (
                    <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 bg-blue-600 text-[var(--text)] font-mono text-[8.5px] px-2 py-0.5 rounded-full font-black animate-pulse">
                      Processing...
                    </div>
                  )}
                ...
                {idx < workflowNodes.length - 1 && (
                  <div className="flex items-center justify-center shrink-0">
                    <ArrowRight className="w-4 h-4 text-[var(--text-subtle)] animate-pulse font-bold hidden md:block" />
                    <span className="text-[var(--text-subtle)] font-extrabold md:hidden text-lg"><ArrowDown className="w-4 h-4" /></span>
                  </div>
                )}
        ...
        <AnimatePresence>
          {isAddingNode && (
            <motion.div
              ...
              className="p-5 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl space-y-4"
            >
              <h4 className="text-xs font-black text-[var(--text)] uppercase tracking-wider flex items-center gap-1 text-blue-400">
              ...
              <form onSubmit={handleAddCustomNode} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <div>
                  <label className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Node Category Type</label>
                  <select
                    value={newNodeType}
                    onChange={(e) => setNewNodeType(e.target.value as any)}
                    className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text)] px-3.5 py-2 rounded-xl mt-1 focus:outline-none"
                  >
                ...
                <div>
                  <label className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Node Name / Type Label</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text)] px-3.5 py-2 rounded-xl mt-1 focus:outline-none"
                  />
                </div>

                <div>
                  <label className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Action Description parameter</label>
                  <input
                    type="text"
                    required
                ...
                <div className="flex gap-2 text-xs">
                  <button
                    type="submit"
                    className="flex-1 py-15 bg-blue-600 hover:bg-blue-500 py-2 font-black text-[var(--text)] rounded-xl transition cursor-pointer"
                  >
                    Sync Node
                  </button>
                  <button
                    ...
                    className="px-4 py-2 bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] text-[var(--text-muted)] rounded-xl transition cursor-pointer"
                  >
                    Cancel
                  </button>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div id="integrations-api-keys-manager" className="bg-[#08080a] border border-[var(--border)] rounded-3xl p-6 flex flex-col justify-between space-y-5 font-sans">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              <div>
                ...
                <p className="text-[10.5px] text-[var(--text-subtle)] font-medium font-sans">Establish secure program access secrets, manage expirations, and rotate tokens.</p>
              </div>

              <button
                onClick={handleGenerateKey}
                className="p-2 border border-blue-500/10 hover:border-blue-500/30 bg-blue-950/15 text-blue-400 hover:text-[var(--text)] rounded-xl text-xs font-bold transition flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="space-y-3">
              {apiKeys.map((key) => {
                ...
                return (
                  <div key={key.id} className="p-3.5 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        <h4 className="text-[11.5px] font-extrabold text-[var(--text)]">{key.name}</h4>
                      ...
                      <div className="text-[9.5px] text-[var(--text-muted)] text-[var(--text-muted)] flex items-center gap-2 mt-0.5">
                        <span className="font-mono bg-[var(--bg-card)] px-1.5 py-0.5 rounded font-black text-blue-400">{key.keyPrefix}</span>
                        <span>•</span>
                    ...
                    <div className="flex items-center gap-1.5 self-end sm:self-auto text-[9.5px]">
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded-lg border flex items-center gap-1 transition ${
                          isRevoked 
                            ? "border-[var(--border)] text-[var(--text-subtle)] cursor-not-allowed" 
                            : "border-[var(--border)] bg-[var(--bg-elevated)]/60 text-[var(--text-muted)] hover:text-[var(--text)]"
                        }`}
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded-lg border transition ${
                          isRevoked 
                            ...
                            : "bg-red-950/10 text-red-400 border-red-500/10 hover:text-[var(--text)]"
                        }`}
                      >
          ...
          <div className="bg-[var(--bg-elevated)]/20 p-3 rounded-xl border border-[var(--border)]">
            <p className="text-[10px] text-[var(--text-subtle)] italic max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              <div>
                ...
                <p className="text-[10.5px] text-[var(--text-subtle)] font-medium">Capture dynamic checkout settlements or lead generation transactions on foreign endpoints.</p>
              </div>
            </div>

            {/* Input target URL */}
            <div className="space-y-1.5">
              <label className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Destination Callback Endpoint URL</label>
              <div className="flex gap-2">
                <input
                  ...
                  className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text)] px-3.5 py-2.5 rounded-xl focus:outline-none focus:border-blue-500/60"
                />
                <button
                  type="button"
                  onClick={handleTestWebhook}
                  className="bg-blue-600 hover:bg-blue-500 text-[var(--text)] text-[11px] font-black px-4 py-2.5 rounded-xl transition flex items-center gap-1 shrink-0 cursor-pointer"
                >
            ...
            <div className="space-y-2">
              <label className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider">Select Webhook Events To Trigger</label>
                    ...
                    <div
                      ...
                      className={`p-2 rounded-xl border flex items-center justify-between cursor-pointer transition ${
                        isChecked 
                          ...
                          : "bg-[var(--bg-card)] border-[var(--border)] text-[var(--text-subtle)] hover:text-[var(--text)]"
                      }`}
                    >
                      <span className="font-mono text-[10.5px] font-semibold">{item.label}</span>
                      <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center ${
                        isChecked ? "bg-blue-600 border-blue-500 text-[var(--text)]" : "border-[var(--border)]"
                      }`}>
            ...
            <div className="space-y-2">
              <span className="text-[9.5px] font-black uppercase text-[var(--text-muted)] tracking-wider flex items-center justify-between">
                <span>Outbound Callback Logs console</span>
                <span className="text-[var(--text-subtle)] text-[8px] font-mono">Real-time deliveries</span>
              </span>
              
              <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-3 max-h-36 overflow-y-auto font-mono text-[9.5px] space-y-2 divider-neutral-900/60">
                  ...
                  webhookLogs.map((log) => (
                    <div key={log.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 pb-1 border-b border-[var(--border)]/60 last:border-0">
                      ...
                      <div className="flex items-center gap-2 text-right self-end sm:self-auto">
                        <span className="text-[var(--text-subtle)] truncate max-w-[120px]">{log.url}</span>
                        <span className={`font-black ${log.status === 200 ? "text-emerald-450" : "text-red-500"}`}>{log.status}</span>
                        <span className="text-[var(--text-subtle)]">({log.latency})</span>
                      </div>

                    </div>
                  ))
                ) : (
                  <p className="text-center text-[var(--text-subtle)] py-3 italic">API webhook transaction backlog empty.</p>
                )}
              </div>
            </div>
      ...
      <div id="integrations-health-monitor" className="bg-[#08080a] border border-[var(--border)] rounded-3xl p-6 space-y-5 font-sans">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-4">
          <div>
            ...
            <p className="text-[10.5px] text-[var(--text-subtle)] font-medium">Track connection latency, sync errors, and live message routing health diagnostics.</p>
          </div>
          ...
          <button
            ...
            className="py-2 px-3.5 bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated)] hover:text-[var(--text)] border border-[var(--border)] rounded-xl text-xs font-bold transition flex items-center gap-1 cursor-pointer text-[var(--text)]"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl">
            <span className="text-[8.5px] text-[var(--text-subtle)] uppercase font-black">Connection status</span>
          ...
          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl">
            <span className="text-[8.5px] text-[var(--text-subtle)] uppercase font-black">Failed Syncs (24h)</span>
            <p className="text-xs font-black text-[var(--text)] mt-1">1 captured</p>
            <p className="text-[9px] text-[#eab308] mt-0.5">Recovered by retry task</p>
          </div>

          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl">
            <span className="text-[8.5px] text-[var(--text-subtle)] uppercase font-black">System Latency</span>
            <p className="text-xs font-black text-[var(--text)] mt-1">112ms avg</p>
            <p className="text-[9px] text-[var(--text-subtle)] mt-0.5 font-mono">100% cloud response</p>
          </div>

          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl">
            <span className="text-[8.5px] text-[var(--text-subtle)] uppercase font-black">Last Successful Sync</span>
            <p className="text-xs font-black text-blue-400 mt-1">Just now</p>
            <p className="text-[9px] text-[var(--text-subtle)] mt-0.5">Automated cron verified</p>
          </div>
        ...
        <div className="space-y-2">
          <h4 className="text-[10px] uppercase font-black text-[var(--text-muted)] tracking-wider">System Connection Warnings / Logs</h4>
          <div className="p-4 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl space-y-3 font-mono text-[10px]">
            ...
            <div className="flex items-start gap-2.5 pb-2.5 border-b border-[var(--border)]/40">
              <span className="p-1 rounded bg-[var(--bg-elevated)] text-emerald-400 shrink-0"> RES</span>
              <div>
                <p className="text-[var(--text)]">Successfully synced lead.captured event package with HubSpot Private CRM Deals pipeline.</p>
                <span className="text-[var(--text-subtle)] text-[8.5px] mt-0.5 block">2026-06-20 11:12:05 UTC | Module: hubspot | Latency: 160ms</span>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <p className="text-[var(--text)]">Target server failed callback validation for event payment.completed. Status code: 500 (Internal Server Error).</p>
                <span className="text-[var(--text-subtle)] text-yellow-500/80 text-[8.5px] mt-0.5 block">2026-06-20 09:30:18 UTC | Module: webhook-api | Retry Queue: Attempt 1 Complete (Waiting next cycle)</span>
              </div>
            </div>
          </div>
        </div>
