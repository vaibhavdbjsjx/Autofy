    ...
    <div id="whatsapp-setup-module" className="space-y-8 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* META ACCESS CARD */}
          <div className="bg-neutral-950/40 border border-var(--border) rounded-3xl p-6 backdrop-blur-md space-y-5">
            <div>
              <h3 className="text-xs font-black text-white uppercase tracking-wider text-blue-400">WhatsApp Business Setup Credentials</h3>
              <p className="text-[10.5px] text-neutral-500 mt-0.5">Enter credentials matching your Meta App configuration platform.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase font-black text-neutral-500 tracking-wider">Business Phone Number</label>
                <input
                  type="text"
                  value={phoneNumber}
                  onChange={(e) => setPhoneNumber(e.target.value)}
                  className="w-full bg-[#0a0a0c] border border-var(--border) p-2.5 rounded-xl text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-blue-500/50"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] uppercase font-black text-neutral-500 tracking-wider font-bold">WhatsApp Business Account ID</label>
                <input
                  type="text"
                  value={businessAccountId}
                  onChange={(e) => setBusinessAccountId(e.target.value)}
                  className="w-full bg-[#0a0a0c] border border-var(--border) p-2.5 rounded-xl text-xs text-white focus:outline-none focus:border-blue-500/50"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] uppercase font-black text-neutral-500 tracking-wider">Meta App ID</label>
                <input
                  type="text"
                  value={metaAppId}
              ...
              <div className="space-y-1.5 relative">
                <label className="text-[10px] uppercase font-black text-neutral-500 tracking-wider">Meta App Secret</label>
                <input
                ...
                <button
                  ...
                  className="absolute right-3.5 bottom-2.5 text-[10.5px] font-bold text-neutral-500 hover:text-white"
                >
            ...
            <div className="p-4 bg-var(--bg-elevated)/30 border border-var(--border) rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] uppercase font-black text-neutral-500">Webhook URL</label>
                  <button
                ...
                <div className="bg-[#050507] p-2 rounded-lg text-[10px] font-mono text-neutral-400 select-all border border-var(--border)">
                  {webhookUrl}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] uppercase font-black text-neutral-500">Webhook Verify Token</label>
                  <button
                ...
                <div className="bg-[#050507] p-2 rounded-lg text-[10px] font-mono text-neutral-400 select-all border border-var(--border) truncate">
                  {verifyToken}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <button
                ...
                className={`px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${
                  connectionStatus === "Connected"
                    ? "bg-var(--bg-elevated) text-neutral-600 border border-transparent cursor-not-allowed"
              ...
              <button
                ...
                className="px-4 py-2.5 text-xs font-bold bg-[#101014] hover:bg-neutral-800 border border-var(--border) rounded-xl text-neutral-300 cursor-pointer"
              >
              ...
              <button
                ...
                className={`px-4 py-2.5 text-xs font-bold bg-neutral-950 hover:bg-red-500/10 hover:border-red-500/20 border border-var(--border) rounded-xl text-neutral-500 hover:text-red-400 cursor-pointer ${
          ...
          <div className="bg-neutral-950/40 border border-var(--border) rounded-3xl p-6 backdrop-blur-md space-y-4">
            <div>
              <h3 className="text-xs font-black text-white uppercase tracking-wider text-blue-400">Message Automation Templates</h3>
              <p className="text-[10.5px] text-neutral-500 mt-0.5">Draft automated response templates for major workflow events.</p>
            </div>
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-[10px] uppercase font-black text-neutral-500">
                  ...
                  <span className="text-neutral-500">Variables supported: {"{{customer_name}}, {{business_name}}"}</span>
                </div>
                <textarea
                  rows={3}
                  value={templates[activeTemplateTab]}
                  onChange={(e) => handleUpdateTemplate(e.target.value)}
                  className="w-full bg-[#0a0a0c] border border-var(--border) p-3.5 rounded-xl text-xs text-white focus:outline-none"
                />
              </div>
              ...
              <div className="flex justify-between items-center p-3.5 bg-var(--bg-elevated)/40 border border-var(--border) rounded-2xl">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-neutral-400">Template synced and approved by Meta Sandbox policy checks.</span>
                </div>
                <button
                  onClick={() => addNewWebhookLog("Message Sent", `Template updated: "${activeTemplateTab} Message" template`, true)}
                  className="px-3.5 py-1.5 text-[10.5px] font-black bg-blue-600 hover:bg-blue-550 text-white rounded-xl cursor-pointer"
                >
                  Save and Sync
                </button>
              </div>
          ...
          <div className="bg-neutral-950/40 border border-var(--border) rounded-3xl p-6 backdrop-blur-md space-y-5">
            <div>
              <h3 className="text-xs font-black text-white uppercase tracking-wider text-blue-400">AI Response Engine Parameters</h3>
              <p className="text-[10.5px] text-neutral-500 mt-0.5 font-sans">Fine-tune confidence limits, booking policies, and out-of-hours fallbacks.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div key={idx} className="flex items-center justify-between p-2.5 bg-var(--bg-elevated)/20 border border-var(--border)/60 rounded-xl">
                    ...
                    <button
                      ...
                      className={`w-7 h-4 rounded-full p-0.5 transition-all outline-none ${
                        item.val ? "bg-blue-600" : "bg-neutral-800"
                      }`}
                    >
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2 bg-var(--bg-elevated)/30 p-4 border border-var(--border) rounded-2xl">
                  ...
                  />
                  <p className="text-[9px] text-neutral-500 text-neutral-500 leading-normal">
                ...
                <div className="space-y-4 bg-var(--bg-elevated)/30 p-4 border border-var(--border) rounded-2xl">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <p className="text-xs font-bold text-white">Outside Business Hours Automation</p>
                      <p className="text-[9.5px] text-neutral-500">Auto replies outside 6AM - 10PM</p>
                    </div>
                    <button
                      onClick={() => setOutOfHoursResponse(!outOfHoursResponse)}
                      className={`w-7 h-4 rounded-full p-0.5 transition-all outline-none ${
                        outOfHoursResponse ? "bg-blue-600" : "bg-neutral-800"
                      }`}
                    >
                      <div className={`w-3 h-3 rounded-full bg-white transition-all ${
                  ...
                  {outOfHoursResponse && (
                    <div className="space-y-1.5">
                      <label className="text-[9.5px] uppercase font-black tracking-wider text-neutral-500 text-neutral-500">Custom Out-Of-Hours reply</label>
                      <textarea
                        rows={2}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#0b0c10] border border-blue-500/20 rounded-3xl p-5 relative overflow-hidden shadow-2xl h-[400px] flex flex-col justify-between">
            
            {/* Handset Header */}
            <div className="flex items-center justify-between border-b border-var(--border) pb-3">
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse" />
                <span className="text-xs font-black text-white uppercase tracking-wider font-sans">Live WhatsApp Tester</span>
              </div>
              <span className="text-[9px] font-mono text-neutral-500">Confidence Score matching feed</span>
            </div>
                  ...
                  <div
                    className={`p-3 rounded-2xl text-xs leading-relaxed ${
                      log.sender === "user"
                        ...
                        : "bg-var(--bg-elevated) text-neutral-200 rounded-bl-none border border-var(--border)"
                    }`}
                  >
              ...
              {isTesterLoading && (
                <div className="flex items-center gap-2 max-w-[80%] p-3 bg-var(--bg-elevated) border border-var(--border) rounded-2xl rounded-bl-none">
                  <div className="flex gap-1">
                    ...
                    <span className="w-1.5 h-1.5 rounded-full bg-neutral-550 animate-bounce delay-150" />
                    <span className="w-1.5 h-1.5 rounded-full bg-neutral-600 animate-bounce delay-300" />
                  </div>
                  <span className="text-[10px] text-neutral-500">RAG matching...</span>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleSendTesterMessage} className="pt-2 border-t border-var(--border) flex gap-2">
              <input
                ...
                className="flex-1 bg-[#050507] border border-var(--border) rounded-xl px-3 text-xs text-white focus:outline-none focus:border-blue-500/50"
              />
              <button
                type="submit"
          ...
          <div className="bg-neutral-950/40 border border-var(--border) rounded-3xl p-5 backdrop-blur-md space-y-4">
            <div className="flex items-center justify-between border-b border-neutral-950 pb-2">
              ...
              <button
                ...
                className="text-[9px] text-neutral-500 hover:text-white"
              >
                Clear logs
              </button>
            </div>
            ...
            <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
              {webhookLogs.map((log) => (
                <div
                  key={log.id}
                  className={`p-3 bg-[#08080a] border border-var(--border) rounded-xl flex gap-3 text-xs justify-between items-start transition-all hover:bg-neutral-950/80 ${
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9.5px] text-neutral-500 font-mono">{log.timestamp}</span>
                    </div>
                    <p className="text-[10.5px] text-neutral-300 leading-normal select-text">{log.details}</p>
                  </div>
                  {log.success ? (
              ...
              {webhookLogs.length === 0 && (
                <p className="text-center text-[10.5px] text-neutral-500 py-6 italic font-sans">No live webhook transactional logs captured yet.</p>
              )}
            </div>
